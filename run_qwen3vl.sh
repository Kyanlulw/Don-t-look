...
ACCELERATE_INIT_DEVICE=cpu python main/qwen3vl_vqa_difficulty.py --input "${INPUT_JSON}" --output "${OUTPUT_JSON}" --image-root "${IMAGE_ROOT}" --range-1 "${RANGE_1}"  --model "${MODEL}" --model-backend qwen --prompt-style qwen --batch-size 8 --max-new-tokens 192 --short-side 448 --long-side 747 --max-pixels 334656 --seed "${SEED}" ${RESUME_FLAG} "$@"
